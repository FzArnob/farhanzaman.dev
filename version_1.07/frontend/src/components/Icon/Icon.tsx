 ...
 *
 * The rest of the site draws its glyphs from icons.ttf, which has no robot, send
 * or shield shape. Pulling the Material web font from Google would add a
 * render-blocking third-party request to a page whose whole point is that it
 * talks to nobody, so the handful of symbols this page needs are inlined instead:
 * they inherit `currentColor`, scale with the font size and cost no request.
 */

const PATHS = {
  /** smart_toy — the bot's face. */
...
interface IconProps {
  name: IconName;
  /** Defaults to 1em so the glyph tracks the surrounding text. */
  size?: number | string;
  className?: string;
}
